...
% Aggregate Euler Equations - Dynamics
figure('Name','Aggregate Euler - with patch')
range_test=[max(Bstar(B_tilde)-1,0)+0.9 Bast(B_tilde_p)+1];
fplot(@(x) Euler_t(x,B_tilde),range_test,'LineWidth',2,'Color',color_t0); hold on; grid on; % axis tight; 
fplot(@(x) Euler_t_p(x,B_tilde_p),range_test,'LineWidth',2,'LineStyle','--','Color',color_t1); 
grid on; yvals=ylim;  xvals=xlim;
line([Bstar(B_tilde) Bstar(B_tilde)],[yvals(1) yvals(2)],'Color','k','LineWidth',1,'LineStyle','--');
line([B_ast_aux B_ast_aux],[yvals(1) yvals(2)],'Color','k','LineWidth',1,'LineStyle','--');
line([B_tilde B_tilde],[yvals(1) yvals(2)],'Color',[0.5 0.5 0.5],'LineWidth',1,'LineStyle','-.'); grid minor;
line([Bhat(B_tilde,B_tilde) Bhat(B_tilde,B_tilde)],[yvals(1) yvals(2)],'Color',[0.5 0.5 0.5],'LineWidth',1,'LineStyle',':');
legend('$\mathcal{E}_{t}$','$\mathcal{E}_{t+1}$','Box','Off','interpreter', 'latex','Location','northwest','AutoUpdate','off')
set(gca,'XTick',Bstar(B_tilde),'XTickLabel',[]);
set(gca,'YTick',[yvals(1) yvals(2)],'YTickLabel',[]);
yTicks = get(gca,'ytick');
...
ax = axis;    %Get left most x-position
text(ax(1)-0.01,yTicks(2),'$\mathcal{E}_t$', 'HorizontalAlignment','Right','interpreter', 'latex');
text(ax(1)-0.06,yTicks(2),'$\mathcal{E}_{t+1},$', 'HorizontalAlignment','Right','interpreter', 'latex');
text(ax(2)+0.2,yTicks(1)-0.05,'$B_{t},B_{t+1}$', 'HorizontalAlignment','Right','interpreter', 'latex');
text(xTicks(1),yTicks(1)-0.1,'$B^{\star}$', 'HorizontalAlignment','Center','VerticalAlignment','Middle','interpreter', 'latex');
text(Bast(B_tilde_p),yTicks(1)-0.1,'$B^{\ast}(\tilde{B})$', 'HorizontalAlignment','Center','VerticalAlignment','Middle','interpreter', 'latex');
text(B_tilde,yTicks(1)-0.1,'$\tilde{B}$', 'HorizontalAlignment','Center','VerticalAlignment','Middle','interpreter', 'latex'); hold on;
text(Bast(B_tilde_p)-0.1,(yTicks(end)+yTicks(1))/5,'$\uparrow$ Efficient Domain of Attraction', 'HorizontalAlignment','Left','VerticalAlignment','top','Rotation',90,'interpreter', 'latex','FontSize',12);
x1 = [ax(1) ax(1) Bstar(B_tilde) Bstar(B_tilde)];
y1 = [yvals(1) yvals(2) yvals(2) yvals(1)];
fill(x1,y1,[0.2 0.2 0.9],'FaceAlpha',0.3);
text(Bstar(B_tilde)-0.1,(yTicks(end)+yTicks(1))/5,'$\uparrow$ Efficient Region ', 'HorizontalAlignment','Left','VerticalAlignment','top','Rotation',90,'interpreter', 'latex','FontSize',12);
x1 = [Bast(B_tilde_p) Bast(B_tilde_p) Bstar(B_tilde) Bstar(B_tilde)];
y1 = [yvals(1) yvals(2) yvals(2) yvals(1)];
fill(x1,y1,[0.8 0.8 0.8],'FaceAlpha',0.3);
...
y1 = [yvals(1) yvals(2) yvals(2) yvals(1)];
fill(x1,y1,[0.5 0.5 0.5],'FaceAlpha',0.3);
text(xvals(2)-0.1,(yTicks(end)+yTicks(1))/5,'$\uparrow$ Hysteresis Region', 'HorizontalAlignment','Left','VerticalAlignment','top','Rotation',90,'interpreter', 'latex','FontSize',12);
text(x1(1)-0.1,(yTicks(end)+yTicks(1))/5,'$\uparrow$ No symmetric eq.', 'HorizontalAlignment','Left','VerticalAlignment','top','Rotation',90,'interpreter', 'latex','FontSize',12);
if printit==1
    orient landscape;
    ax = gca;
    exportgraphics(ax,[params.folder 'F_aggregate_transition.pdf'],'BackgroundColor','none');
end
