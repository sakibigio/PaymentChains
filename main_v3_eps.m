...
if plotit==1
    ...
    if printit==1
        orient landscape;
        ax = gca;
        exportgraphics(ax,'TFP_example.eps','BackgroundColor','none');
    ...
    legend('Low $\mu$','High $\mu$','Interpreter','Latex');
     if printit==1
        orient landscape;
        % saveas(gcf, 'Dist_example', 'eps')
        ax = gca;
        exportgraphics(ax,'Dist_example.eps','BackgroundColor','none');
...
if printit==1
    orient landscape;
    % saveas(gcf,'F_transition','eps');
    ax = gca;
    exportgraphics(ax,'F_transition.eps','BackgroundColor','none');
end
...
if printit==1
    orient landscape;
   %  saveas(gcf,'F_Japan','eps');
     ax = gca;
    exportgraphics(ax,'F_Japan.eps','BackgroundColor','none');
end
...
if printit==1
    orient landscape;
   %  saveas(gcf,'F_SSRate','eps');
   ax = gca;
    exportgraphics(ax,'F_SSRate.eps','BackgroundColor','none');
end
...
for vv=1:numel(varlist)
  ...
  %  plot(1:T,var_v,'LineWidth',linethick,'Color',color_t0); xlabel('Time','Interpreter','latex','FontSize',fontsize); 
    ...
    orient landscape;
    eval(['saveas(gcf,''' printlist{vv} ''',''eps'');']);
end
